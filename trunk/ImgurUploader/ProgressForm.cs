...
namespace ImgurUploader
{
    public partial class ProgressForm : Form
    {
        ...
        {
            UploaderPreferences tPref = UploaderPreferences.Default;
            formatCombo.SelectedIndex = 0;
            if (!String.IsNullOrEmpty(tPref.DefaultExport))
                foreach(var tI in formatCombo.Items)
                    if (tI.ToString() == tPref.DefaultExport) { formatCombo.SelectedItem = tI; break; }
            this.formatCombo.SelectedIndexChanged += new System.EventHandler(this.formatCombo_SelectedIndexChanged);

            linkCheck.Checked = tPref.IncludeLinks;
        ...
        {
            UploaderPreferences tPref = UploaderPreferences.Default;
            tPref.DefaultExport = formatCombo.SelectedItem.ToString();
            tPref.Save();
        }
        ...
        {
            UploaderPreferences tPref = UploaderPreferences.Default;
            tPref.IncludeLinks = linkCheck.Checked;
            tPref.Save();
        }
    }
}
